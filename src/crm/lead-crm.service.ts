...
export class LeadCrmService {
  ...
  async getLivePulse(companyId: string) {
    ...
    const [
      ...
      this.prisma.companyMember.count({
        where: { companyId, user: { lastSeenAt: { gte: onlineSince } } },
      }),
      ...
      this.prisma.offer.count({
        where: { companyId, status: OfferStatus.PENDING },
      }),
      this.prisma.offer.count({
        where: { companyId, createdAt: { gte: since } },
      }),
    ...
    return {
      ...
      feed: activities.map((row) => ({
        ...
        requestTitle: row.lead.request.title,
      })),
    };
  }
